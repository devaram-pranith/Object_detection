...
class Yolov3:
    ...
    def bounding_box(self, detections):
        ...
        for i in detections:
            for j in i:
                ...
                if confidence_ > Threshold:
                    object_count[self.classes[class_]] = object_count.get(self.classes[class_], 0) + 1
        ...
        return final_box, cordinates, confidence_score, ids, object_count

    def predictions(self, prediction_box, bounding_box, confidence, class_labels, width_ratio, height_ratio,
                    end_time,
                    image):
...
def parse_args():
    ...
    parser.add_argument('--video', type=str, default='yolo_test.mp4', help='Path to input video file')
...
if __name__ == "__main__":
    ...
    try:
        ...
        while cap.isOpened():
            ...
            res, frame = cap.read()
            if res:
                outcome, object_count = obj.Inference(image=frame, original_width=width, original_height=height)
                for obj_class, count in object_count.items():
                    print(f"Number of {obj_class} detected in this frame: {count}")
